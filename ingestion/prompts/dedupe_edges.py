"""Verbatim port of Graphiti's edge dedup + contradiction sweep prompt.
...
into a single call; the ``invalidate_edges`` module re-uses this same prompt
with a slimmer response model when only the contradiction half is needed.
...
# (existing_edges, edge_invalidation_candidates, new_edge). Prompt text,
# examples, and rules unchanged.

from __future__ import annotations

from typing import Any

from .models import EdgeDuplicate, Message


def build_prompt(context: dict[str, Any]) -> list[dict[str, str]]:
    """Render the combined duplicate + contradiction prompt.

    Required context keys:
      - ``existing_edges`` — JSON / string of EXISTING FACTS with idx prefix
      - ``edge_invalidation_candidates`` — JSON / string of INVALIDATION
        CANDIDATES with continuing idx numbering
      - ``new_edge`` — string/JSON of the new fact
    """
    messages = [
        Message(
            role="system",
...
# Re-export so callers can `from .dedupe_edges import build_prompt, EdgeDedupResponse`.
EdgeDedupResponse = EdgeDuplicate


__all__ = ["EdgeDedupResponse", "build_prompt"]
