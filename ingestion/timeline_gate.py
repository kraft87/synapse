...
QUOTED MATERIAL — this timeline is the USER's autobiography, and turns often contain pasted third-party content: someone else's email or message, a meeting/interview/legal transcript, a chat log, an article. Happenings narrated INSIDE that material happened to someone else. NEVER emit them as events, no matter how personal, dated, or first-person the quoted text reads (a quoted speaker's "I started a new job" is THEIR event, not the user's). Only the user's own work ON the material stays eligible ("delivered the corrected transcript", "fixed the parser"). Distinguish: something a third party said ABOUT the user ("my accountant told me to...") is still about the user and stays eligible.

PLANNED IS NOT DONE. Never write a happening the turn only plans, intends, hopes for, schedules, requests, or lists as a step — an intent, target, estimate, expectation, or anchor is NOT an outcome. A figure a third party states ("company X will pay $N", "the posting goes to $M", "the shop quoted $Q") is a stated number, NOT a received offer, an agreed price, or money in hand. Intent to negotiate, apply, buy, or ask is NOT the negotiation, application, purchase, or answer. The META-ACT is separate and may itself be complete: booking, scheduling, or submitting IS a completed booking/submission ("booked the MRI for June 16 (meaning 2026-06-16)"), never the thing booked. Items listed under a future day ("tomorrow: ...", "next week: ...", "plan for Friday: ...") are a TO-DO list, not a log — never write one as done. A DECISION to do something later IS a happening and still gets emitted ("decided to run the migration next weekend"); what is banned is writing the future thing itself as already carried out. When completion is genuinely ambiguous, emit the weaker happening — the decision that WAS reached, the booking that WAS made — never the stronger one; if there is no weaker happening either, emit nothing.
BAD:  "received a job offer of $118,000 from Vantor Systems and planned to negotiate toward the band max of $141,000" (a recruiter only stated what the role pays; no offer exists, no negotiation happened)
GOOD: "decided to negotiate toward the top of the Vantor Systems posted band of $118,000 to $141,000" — or {"events": []} if nothing was decided either.
