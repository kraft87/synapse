...
**OAuth callers identify themselves, not their host.** MCP clients on the interactive OAuth/OIDC lane (the claude.ai connector) run no PreToolUse hook, so they send no `surface` at all — under the first cut of 053 a fully authenticated owner identity resolved to UNKNOWN and got the restricted serve. They carry stronger evidence than a hostname though: an identity this server verified and allowlisted. `mcp_server/server._caller_surface` resolves it to the surface id **`oauth:<login>`** (the same lowercased claim the `_UserAllowlist` gate reads: `login` on the GitHub leg, `preferred_username`/`email` on OIDC) and looks that up in `surfaces` like any other id. Fail-closed is unchanged — **no `oauth:<login>` row still means restricted**, and nothing auto-creates a row or infers trust from a successful login; the operator registers it. The derived id **overrides** a client-supplied `surface`, because a verified token identity outranks a string the caller typed about itself. Machine-token callers are untouched: that token says only "a Synapse client", never which host, so its hook-injected param stays the only evidence available. Plain-HTTP routes are machine-token-only and unaffected.
