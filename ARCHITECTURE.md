...
- **A device token per surface (054)** — `token_hash` (sha256 hex, unique where non-null), `status IN ('pending','approved','revoked')` defaulting to `pending`, a display-only `label`, a `pair_code`, `last_seen_at`, and the machine's self-declared `requested_trust`/`requested_projects`.
...
**Only `status='approved'` serves.** A pending device holds a real token that authenticates as nothing; a revoked one holds a token matching no row. A non-approved row resolves to `UNKNOWN_SURFACE` rather than "restricted but known", because `known` is what lets a restricted write default to `work-safe` — an unapproved device must not be able to widen a note's later audience.

#### Enrollment, approval, and the request

`POST /surfaces/enroll` mints a device token and inserts a row, under one advisory lock so two simultaneous first-enrollments cannot both bootstrap. If this Synapse has **no approved device with a token**, the first one in is approved at `full` — nobody exists who could approve it, and the branch is unreachable ever after. Every later enrollment lands `pending` with a 6-character pair code. The `label` is display only and is deliberately never matched against an existing row: binding an enrollment to a self-reported name would reintroduce hostname spoofing through the back door ("enroll as `trusted-laptop`, inherit its grant").

The plugin's install prompt asks **personal or work** (`SYNAPSE_MACHINE_ROLE`, default personal) and sends the answer as `requested_trust`. It is a **request, never a grant**: stored on the pending row, printed next to the pair code on *both* ends, and used as the DEFAULT that `approve` fills in — so approving is a one-word confirmation of a role already on screen rather than a second decision typed from memory. Explicit `trust`/`allowed_projects` on approve override it in either direction, and a restricted approve that names no projects inherits the union of existing approved restricted allowlists (a second work machine should see the same work projects as the first). A device that stated nothing falls back to `restricted` with an empty allowlist. One deliberate edge: a **first-ever** device requesting `restricted` does *not* bootstrap — auto-approving a device that cannot reach the admin routes would leave the deployment unadministrable, so it lands pending and the operator runs `scripts/surface_admin.py bootstrap`.

The pair code appears on both ends by design. The enrolling machine prints a pairing block instead of a board (it is served nothing, and an unexplained empty session start is worse than a restricted one); the trusted machine's board carries `N device(s) pending approval: work-laptop (K7Q2M9, requests: restricted)`. An approval only one side can see cannot be verified.

#### Two gates, and their asymmetry is the point

| Gate | Admits | Guards |
| --- | --- | --- |
| **client** (`_machine_authorized`) | root token, or any APPROVED device token | `/ingest`, `POST /recall`, `GET /context`, skills / config / timeline / preferences / private-session / remember-spool routes, `POST /surfaces/enroll` |
| **admin** (`_admin_authorized`) | APPROVED **full-trust device** token only — **never the root token** | `/surfaces` list + `PUT`/`DELETE`, `/surfaces/approve`, `/surfaces/mint`, every `/dash/api/*` route |

Every machine holds the root token long enough to enroll. If root could also approve, a machine could self-approve and trust-on-first-use would be decoration — hence: **the credential that can enroll cannot bless.** `/dash/*` had to move for the same reason (it serves the whole corpus unfiltered and can flag/edit, so leaving it on root would be a bypass around the entire filter); the dashboard login flow now mints a full-trust device token for the OAuth-allowlisted identity (`dash:<login>`, rotated per login, one row per identity) instead of handing the browser the root token. `issue_machine_token` refuses device tokens: leaves do not fetch the root.

**Break-glass is not a token.** `scripts/surface_admin.py` (list / approve / mint / revoke / bootstrap) talks to Postgres over a direct DSN, so recovery from "the only full-trust device was wiped" requires shell access on the DB host — a strictly higher bar than holding a bearer.

**Root verification never touches the database** (`mcp_server/auth_tokens.SynapseTokenVerifier`): constant-time compare, no I/O. The services on the Docker host authenticate with the root token to reach `/ingest` and the internal write paths, so a verifier that had to read PG to admit root would turn every PG hiccup into a total auth outage on the one lane that repairs things. Device tokens do read PG (and stamp `last_seen_at`, throttled and best-effort — auth must not depend on a WRITE succeeding).
...
| Variable | Default | Description |
|----------|---------|-------------|
| `SYNAPSE_MACHINE_TOKEN` | optional | ROOT bearer: gates `/ingest` + the internal write lanes, and is the **enrollment credential** a new device trades for its own token ([§7.5](#75-audience-scoping--credential-bound-surface-trust-schema-053--054)). Deliberately NOT sufficient to approve devices or reach `/dash/api`. **Unset → server runs open** |
