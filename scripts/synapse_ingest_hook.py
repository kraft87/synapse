...
Works for any agent frontend that drives the real `claude` CLI, which fires this hook.
...
INGEST_URL = os.environ.get("SYNAPSE_INGEST_URL", "http://localhost:8765/ingest")
...
def _ship(transcript_path: str) -> None:
    ...
    try:
        ...
        if not records:
            return
        body = json.dumps({"records": records, "source": "hook"}).encode()
        req = urllib.request.Request(
            INGEST_URL, data=body, headers={"Content-Type": "application/json"}, method="POST"
        )
        with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:  # trusted LAN URL
