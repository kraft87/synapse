...
# ---------------------------------------------------------------------------
# The pending-device nudge (schema 054)
# ---------------------------------------------------------------------------


@pytest.fixture()
def no_surfaces(conn):
    """This file registers surfaces without tearing them down, so the pending-device
    tests below (which assert on ABSENCE) clean up around themselves."""
...
def test_a_full_trust_board_announces_pending_devices_with_their_pair_code(no_surfaces, db_url):
    """A pending device is served nothing and cannot announce itself, so the board is
    what tells the operator it is waiting. The code has to appear HERE as well as on the
    enrolling machine — an approval where only one side shows it cannot be verified."""
    ...
    register_device(
        ...
        trust="restricted",
        status="pending",
        surface_id="dev-waiting",
        label="work-laptop",
        pair_code="AB12CD",
    )
    text = build_board(db_url, None, trust=FULL_TRUST)["text"]
    assert "1 device(s) pending approval" in text
    assert "work-laptop (AB12CD)" in text


def test_the_nudge_shows_the_role_the_device_asked_for(no_surfaces, db_url):
    """So approving is a one-word confirmation of something already on screen, rather
    than a second decision typed from memory."""
    from tests.helpers.surfaces import register_device
    ...
    text = build_board(db_url, None, trust=FULL_TRUST)["text"]
    assert "work-laptop (AB12CD, requests: restricted)" in text


def test_a_restricted_board_never_shows_a_pair_code(no_surfaces, db_url):
    """A restricted surface must not learn that other machines exist, and must certainly
    not hold a code that could approve one."""
    from tests.helpers.surfaces import register_device
    ...
    sid = register_restricted(no_surfaces, ["alpha"])
    text = build_board(db_url, None, sid)["text"]
    assert "AB12CD" not in text and "pending approval" not in text


def test_no_pending_devices_means_no_extra_line(no_surfaces, db_url):
    text = build_board(db_url, None, trust=FULL_TRUST)["text"]
